...
final timeBlockDateProvider = StateProvider<DateTime>((ref) {
  final now = DateTime.now();
  return DateTime(now.year, now.month, now.day);
});

/// Scheduled time blocks for the selected date.
final timeBlocksProvider = StateProvider<List<TimeBlock>>((ref) => []);
...
class _TimeBlockingPageState extends ConsumerState<TimeBlockingPage> {
  ...
  void _onTaskDropped(HomeTask task, double localDy) {
    ...
    ref.read(timeBlocksProvider.notifier).state = [...blocks, newBlock];
  }
  ...
  void _onBlockDragUpdate(TimeBlock block, double newDy) {
    ...
    ref.read(timeBlocksProvider.notifier).state = updated;
  }

  void _removeBlock(String blockId) {
    final blocks = ref.read(timeBlocksProvider);
    ref.read(timeBlocksProvider.notifier).state =
  ...
  Future<void> _pickDate(BuildContext context) async {
    ...
    if (picked != null) {
      ref.read(timeBlockDateProvider.notifier).state = picked;
    }
  }
