...
void main() {
  ...
  group('MembersNotifier', () {
    ...
    test('addMember adds to state', () {
      ...
      expect(container.read(membersProvider).valueOrNull?.length, 1);
      expect(
        container.read(membersProvider).valueOrNull?.first.name,
        'Alice',
      );
    });
    ...
    test('removeMember removes by ID', () async {
      ...
      expect(
        container.read(membersProvider).valueOrNull?.isEmpty,
        isTrue,
      );
    });
    ...
    test('updateRole changes member role', () async {
      ...
      expect(
        container.read(membersProvider).valueOrNull?.first.role,
        TeamRole.admin,
      );
    });
    ...
    test('loadMembers returns empty list when API unavailable', () async {
      ...
      expect(
        container.read(membersProvider).valueOrNull,
        isEmpty,
      );
    });
  });
  ...
  group('InvitesNotifier', () {
    ...
    test('sendInvite adds invite to state', () async {
      ...
      final invites = container.read(invitesProvider).valueOrNull ?? [];
    ...
    test('revokeInvite changes status to revoked', () async {
      ...
      final id =
          container.read(invitesProvider).valueOrNull!.first.id;
      notifier.revokeInvite(id);
      expect(
        container.read(invitesProvider).valueOrNull?.first.status,
  ...
  group('StandupNotifier', () {
    ...
    test('submitStandup adds entry to front of list', () async {
      ...
      expect(container.read(standupProvider).valueOrNull?.length, 1);
      expect(
        container.read(standupProvider).valueOrNull?.first.id,
        's1',
      );
    });
    ...
    test('loadHistory returns empty list when API unavailable', () async {
      ...
      await _pumpEventQueue();
      expect(
        container.read(standupProvider).valueOrNull,
        isEmpty,
      );
    });
  });
