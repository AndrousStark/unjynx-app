/// UNJYNX AI - Claude chat, auto-schedule, weekly insights.
library feature_ai;

export 'src/ai_plugin.dart';
export 'src/domain/models/ai_insight.dart';
export 'src/domain/models/chat_message.dart';
export 'src/domain/models/schedule_suggestion.dart';
export 'src/presentation/pages/ai_chat_page.dart';
export 'src/presentation/pages/ai_insights_page.dart';
export 'src/presentation/pages/ai_schedule_page.dart';
export 'src/presentation/providers/ai_providers.dart';
