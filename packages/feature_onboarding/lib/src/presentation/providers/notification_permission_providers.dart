...
/// Notifier for the notification permission status string.
class NotificationPermissionStatusNotifier extends Notifier<String> {
  @override
  String build() => 'not_asked';
...
final notificationPermissionStatusProvider =
    NotifierProvider<NotificationPermissionStatusNotifier, String>(
...
Future<bool> requestNotificationPermission(WidgetRef ref) async {
  ...
  if (port != null) {
    ...
    ref.read(notificationPermissionStatusProvider.notifier).set(
  ...
  ref.read(notificationPermissionStatusProvider.notifier).set('granted');
  return true;
}
