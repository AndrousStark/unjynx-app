...
class PersonalizationNotifier extends Notifier<PersonalizationState> {
  ...
  Future<void> savePreferences() async {
    final s = state;
    final port = ref.read(userPreferencesPortProvider);

    // Save in parallel for speed.
    await Future.wait([
      if (s.identity != null) port.saveIdentity(s.identity!),
      if (s.goals.isNotEmpty) port.saveGoals(s.goals.toList()),
      if (s.channelPrefs.isNotEmpty)
        port.saveChannelPrefs(s.channelPrefs),
      port.saveContentCategories(
        s.contentCategories,
        s.contentDeliverAt,
      ),
    ]);
