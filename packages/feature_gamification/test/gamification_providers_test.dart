...
void main() {
  ...
  group('Providers (defaults)', () {
    ...
    test('xpDataProvider returns empty fallback when no API', () async {
      final xp = await container.read(xpDataProvider.future);
      expect(xp.totalXp, 0);
      expect(xp.level, 1);
    });

    test('achievementsProvider returns empty when no API', () async {
      final list = await container.read(achievementsProvider.future);
      expect(list, isEmpty);
    });

    test('leaderboardProvider returns empty when no API', () async {
      final list = await container.read(leaderboardProvider.future);
      expect(list, isEmpty);
    });

    test('activeChallengesProvider returns empty when no API', () async {
      final list = await container.read(activeChallengesProvider.future);
      expect(list, isEmpty);
    });

    test('partnersProvider returns empty when no API', () async {
      final list = await container.read(partnersProvider.future);
      expect(list, isEmpty);
    });

    test('sharedGoalsProvider returns empty when no API', () async {
